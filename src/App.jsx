...
export default function App() {
  ...
  const saveLog = () => {
    ...
    if (editFlightId) {
      setFlights(fl => fl.map(f => f.id === editFlightId ? entry : f));
    } else {
  ...
  const deleteFlight = (id) => {
    setFlights(fl => fl.filter(f => f.id !== id));
    setConfirmDel(null);
  };
  ...
  const addNewCrew = () => {
    setAddCrewErr("");
    if (!newCrew.id.trim() || !newCrew.nickname.trim()) { setAddCrewErr("ID 和英文名為必填 ID & Nickname required"); return; }
    if (crew.find(m=>m.id===newCrew.id.trim())) { setAddCrewErr("此 ID 已存在 ID already exists"); return; }
    ...
    <div style={{display:"flex",flexDirection:"column",height:"100vh",overflow:"hidden"}}>
      <div style={{padding:"18px 16px 12px",background:c.card,borderBottom:`1px solid ${c.border}`,flexShrink:0}}>
        ...
        <div style={{position:"relative"}}>
          ...
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="ID / 姓名 / Nickname / 備忘關鍵字..."
      ...
      <div style={{flex:1,overflowY:"auto",padding:"14px 16px 80px"}}>
        ...
        <div style={{marginTop:24,background:c.card,border:`1px dashed ${c.border}`,borderRadius:16,padding:16}}>
          <div style={{fontSize:10,letterSpacing:3,color:c.accent,fontWeight:700,marginBottom:12}}>新增組員 ADD CREW</div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8}}>
            <input value={newCrew.id} onChange={e=>setNewCrew(n=>({...n,id:e.target.value}))} placeholder="員工 ID *"
              style={{...inp,fontSize:13,padding:"9px 12px"}}/>
            <input value={newCrew.nickname} onChange={e=>setNewCrew(n=>({...n,nickname:e.target.value}))} placeholder="英文名 Nickname *"
              style={{...inp,fontSize:13,padding:"9px 12px"}}/>
            <input value={newCrew.name} onChange={e=>setNewCrew(n=>({...n,name:e.target.value}))} placeholder="姓名 (中文/日文)"
              style={{...inp,fontSize:13,padding:"9px 12px"}}/>
            <input value={newCrew.seniority} onChange={e=>setNewCrew(n=>({...n,seniority:e.target.value}))} placeholder="期別 e.g. 24G"
              style={{...inp,fontSize:13,padding:"9px 12px"}}/>
          </div>
          {addCrewErr&&<div style={{color:"#FF453A",fontSize:12,marginBottom:8}}>{addCrewErr}</div>}
          <button onClick={addNewCrew} style={{width:"100%",background:c.accent,color:c.adk,border:"none",borderRadius:12,padding:"10px",fontSize:14,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>
            + 新增 Add Member
          </button>
        </div>
      </div>
