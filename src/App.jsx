...
```
    ...
    <Sect label="預設值 DEFAULTS" c={c}>
      <div style={{ background: c.card, border: `1px solid ${c.border}`, borderRadius: 14, padding: 14, marginBottom: 8 }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={() => setDefaultAircraft("")} style={{ background: !defaultAircraft ? c.accent : c.pill, color: !defaultAircraft ? c.adk : c.sub, border: "none", borderRadius: 10, padding: "8px 12px", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>無 None</button>
          {AIRCRAFT.map(a => (
            <button key={a} onClick={() => setDefaultAircraft(defaultAircraft === a ? "" : a)} style={{ flex: 1, background: defaultAircraft === a ? c.accent : c.pill, color: defaultAircraft === a ? c.adk : c.sub, border: "none", borderRadius: 10, padding: "8px", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>{a}</button>
          ))}
        </div>
      </div>
      <div style={{ background: c.card, border: `1px solid ${c.border}`, borderRadius: 14, padding: 14 }}>
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 8 }}>
          <button onClick={() => setDefaultPosition("")} style={{ background: !defaultPosition ? c.accent : c.pill, color: !defaultPosition ? c.adk : c.sub, border: "none", borderRadius: 8, padding: "6px 12px", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>無 None</button>
          {POSITIONS.map(p => (
            <button key={p} onClick={() => setDefaultPosition(defaultPosition === p ? "" : p)} style={{ background: defaultPosition === p ? c.accent : c.pill, color: defaultPosition === p ? c.adk : c.sub, border: "none", borderRadius: 8, padding: "6px 10px", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>{p}</button>
          ))}
        </div>
      </div>
    </Sect>
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 12 }}>
          ...
          {customTags.map(t => (
            <span key={t} style={{ background: c.accent + "22", color: c.accent, borderRadius: 20, padding: "5px 8px 5px 12px", fontSize: 12, fontWeight: 700, display: "flex", alignItems: "center", gap: 4 }}>
              {t}
              <button onClick={() => setCustomTags(ct => ct.filter(x => x !== t))} style={{ background: "none", border: "none", color: "#FF453A", fontSize: 14, cursor: "pointer", padding: "0 2px", lineHeight: 1 }}>×</button>
            </span>
          ))}
        </div>
        <div style={{ display: "flex", gap: 8 }}>
          <ClearableInput
            ...
            c={c}
          />
          <button onClick={addCustomTag} style={{ background: c.accent, color: c.adk, border: "none", borderRadius: 10, padding: "9px 16px", fontSize: 13, fontWeight: 700, cursor: "pointer", flexShrink: 0 }}>+ 新增</button>
        </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {routes.map(r => (
              <div key={r.id} style={{ display: "flex", alignItems: "center", gap: 8, background: c.cardAlt, borderRadius: 10, padding: "8px 10px" }}>
                ...
                <button onClick={() => setRoutes(rs => rs.filter(x => x.id !== r.id))} style={{ marginLeft: "auto", background: "none", border: "none", color: "#FF453A", cursor: "pointer", fontSize: 14, padding: "0 4px" }}>×</button>
              </div>
            ))}
          </div>
        )}
    ...
    <Sect label="危險區域 DANGER ZONE" c={c}>
      {confirmClear ? (
        <div style={{ background: "rgba(255,69,58,0.1)", border: "1px solid rgba(255,69,58,0.4)", borderRadius: 14, padding: 16 }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={() => { onImport({ flights: [] }); setConfirmClear(false); }} style={{ flex: 1, background: "#FF453A", color: "#fff", border: "none", borderRadius: 10, padding: "11px", fontSize: 13, fontWeight: 800, cursor: "pointer" }}>確認清除</button>
            <button onClick={() => setConfirmClear(false)} style={{ flex: 1, background: c.pill, color: c.sub, border: "none", borderRadius: 10, padding: "11px", fontSize: 13, cursor: "pointer" }}>取消</button>
          </div>
        </div>
      ) : (
...
```
      ...
      {addR && (
        <div style={{ background: c.cardAlt, border: `1px solid ${c.border}`, borderRadius: 12, padding: 12, marginBottom: 10 }}>
          ...
          <div style={{ display: "flex", gap: 6, marginBottom: 8 }}>
            {AIRCRAFT.map(a => (
              <button key={a} onClick={() => setRf(r => ({ ...r, ac: a }))} style={{ flex: 1, background: rf.ac === a ? c.accent : c.pill, color: rf.ac === a ? c.adk : c.sub, border: "none", borderRadius: 8, padding: "7px", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>{a}</button>
            ))}
          </div>
          <div style={{ display: "flex", gap: 8 }}>
    ...
    <Sect label="職位 POSITION" c={c}>
      ...
      <ClearableInput value={form.position} onChange={e => setForm(f => ({ ...f, position: e.target.value }))} placeholder="或自行輸入..." autoComplete="off" style={inp} c={c} />
    </Sect>
    ...
    <Sect label="備忘 MEMO" c={c}>
      <ClearableTextarea value={form.memo} onChange={e => setForm(f => ({ ...f, memo: e.target.value }))} rows={3} placeholder="這次飛行的備忘..." style={{ ...inp, resize: "vertical" }} c={c} />
    </Sect>
...
const sections = [
{ emoji: “✈”, title: “什麼是 CrewLog？”, en: “What is CrewLog?”, content: “CrewLog 是你的私人空中生存指南。記錄合飛組員，留下備忘，用紅黃綠燈標記好壞，幫助你下次飛行前做好心理準備。\n\nCrewLog is your private cabin crew companion — log who you fly with, leave notes, and mark them green, yellow, or red so you’re never caught off-guard again.” },
{ emoji: “🔒”, title: “隱私設計”, en: “Privacy”, content: “飛行紀錄 (備忘、航班) 是完全私人的 — 只有你看得到，不會同步給其他用戶。\n\n組員的基本資料 (名字、期別) 和紅黃綠燈、標籤則是大家共享的，讓整個 app 的資料保持最新。\n\nYour flight logs and memos are private (only you see them). Crew info, status lights, and tags are shared so everyone benefits.” },
{ emoji: “🔴🟡🟢”, title: “紅黃綠燈”, en: “Status Lights”, isList: true, content: [{ icon: “🟢”, label: “推薦 Great!”, desc: “好合作、專業、值得信任的組員” }, { icon: “🟡”, label: “普通 Neutral”, desc: “一般，沒有特別好或壞” }, { icon: “🔴”, label: “注意 Warning”, desc: “需要注意，可搭配備忘說明原因” }] },
{ emoji: “🏷”, title: “標籤 Tags”, en: “Tags”, isList: true, content: [{ icon: “#好咖”, desc: “優秀的組員，合作愉快” }, { icon: “#難搞”, desc: “不好合作，注意一下” }, { icon: “#細心”, desc: “工作細心，注意到小細節” }, { icon: “#新人”, desc: “新組員，需要多幫忙” }, { icon: “#好笑”, desc: “幽默風趣，飛起來很開心” }, { icon: “#專業”, desc: “工作態度專業” }, { icon: “#八卦”, desc: “愛說話，要注意嘴型 👀” }, { icon: “#準時”, desc: “很守時，不拖拖拉拉” }] },
{ emoji: “📝”, title: “如何新增飛行紀錄”, en: “How to Log a Flight”, content: “1. 點右下角的 ＋ 按鈕，或點組員卡片上的 ＋\n2. 搜尋組員名字、ID 或 Nickname\n3. 選擇日期、航班、機型、職位\n4. 設定紅黃綠燈和標籤\n5. 寫下備忘，然後儲存！\n\nHit + → search crew → fill in details → save. Easy.” },
{ emoji: “🔍”, title: “搜尋功能”, en: “Search”, content: “搜尋欄可以搜尋：\n• 組員 ID (員工號碼)\n• 中文姓名\n• 英文 Nickname\n• 飛行備忘的內容 (輸入兩個字以上)\n\n有備忘符合的組員會顯示 📝 提示。” },
{ emoji: “👤”, title: “組員頁面”, en: “Crew Profile”, content: “點任何組員可以進入個人頁面：\n• 查看你們所有的合飛紀錄\n• 編輯組員基本資料（大家共享）\n• 新增長期筆記（大家共享）\n• 快速設定紅黃綠燈\n• 編輯或刪除個別飛行紀錄” },
{ emoji: “⬇”, title: “備份資料”, en: “Backup”, content: “設定頁面的「備份」可以將所有資料下載成 JSON 檔案。建議定期備份，以防萬一。\n\nGo to Settings → Backup to download all your data as a JSON file.” },
];
...
onBack={onBack}
c={c}
right={<span style={{ fontSize: 12, color: c.sub, fontWeight: 700, background: c.pill, borderRadius: 8, padding: “4px 10px” }}>{flights.length} 筆</span>}
/>
...
<ClearableInput value={search} onChange={e => setSearch(e.target.value)} placeholder=“搜尋組員姓名或備忘…” autoComplete=“off” style={inp} c={c} />
</div>
</div>
...
<div style={{ textAlign: “center”, padding: “64px 0”, color: c.sub, fontSize: 14 }}>找不到符合「{search}」的紀錄</div>
) : (
months.map(month => (
<div key={month} style={{ marginBottom: 28 }}>
<div style={{ display: “flex”, alignItems: “center”, gap: 10, marginBottom: 12 }}>
<span style={{ fontSize: 11, fontWeight: 800, letterSpacing: 3, color: c.accent, flexShrink: 0 }}>{month}</span>
...
return (
<div key={f.id} style={{ background: c.card, border: `1px solid ${c.border}`, borderLeft: `3px solid ${si ? si.color : c.border}`, borderRadius: 14, padding: “12px 14px”, display: “flex”, gap: 12, alignItems: “flex-start” }}>
<div style={{ flexShrink: 0, width: 36, paddingTop: 2, textAlign: “center” }}>
<div style={{ fontSize: 13, fontWeight: 800, color: c.text, lineHeight: 1 }}>{f.date ? f.date.slice(8) : “—”}</div>
<div style={{ fontSize: 9, color: c.sub, fontWeight: 600, marginTop: 2 }}>{f.date ? [“SUN”,“MON”,“TUE”,“WED”,“THU”,“FRI”,“SAT”][new Date(f.date).getDay()] : “”}</div>
</div>
<div style={{ width: 1, alignSelf: “stretch”, background: c.border, flexShrink: 0 }} />
<div style={{ flex: 1, minWidth: 0 }}>
<div onClick={() => m && onGoProfile(m.id)} style={{ display: “flex”, alignItems: “center”, gap: 7, marginBottom: hasMemo ? 7 : 0, cursor: m ? “pointer” : “default” }}>
{si ? <span style={{ fontSize: 13, lineHeight: 1, flexShrink: 0 }}>{si.emoji}</span> : <Dot status={null} sz={8} c={c} />}
<span style={{ fontWeight: 800, fontSize: 15, color: c.text }}>{m ? m.nickname : `#${f.crewId}`}</span>
{m?.name && <span style={{ fontSize: 12, color: c.sub, overflow: “hidden”, textOverflow: “ellipsis”, whiteSpace: “nowrap” }}>{m.name}</span>}
{f.flightNum && <span style={{ marginLeft: “auto”, fontSize: 10, color: c.accent, fontWeight: 700, background: c.pill, borderRadius: 6, padding: “1px 6px”, flexShrink: 0 }}>{f.flightNum}</span>}
</div>
{hasMemo && (
<div style={{ fontSize: 12, color: c.sub, lineHeight: 1.55, background: c.cardAlt, borderRadius: 8, padding: “6px 10px”, display: “-webkit-box”, WebkitLineClamp: 2, WebkitBoxOrient: “vertical”, overflow: “hidden” }}>
📝 {f.memo}
</div>
)}
</div>
<button onClick={() => onEdit(f)} style={{ background: “none”, border: “none”, color: c.sub, cursor: “pointer”, fontSize: 13, padding: “2px 4px”, flexShrink: 0, alignSelf: “flex-start” }}>✏</button>
</div>
);
})}
</div>
...
<div onClick={() => setView(“mylog”)} style={{ background: c.pill, borderRadius: 12, padding: “8px 12px”, marginBottom: 12, display: “flex”, alignItems: “center”, justifyContent: “space-between”, cursor: “pointer” }}>
<div style={{ display: “flex”, alignItems: “center”, gap: 8 }}>
...
<ClearableInput value={search} onChange={e => setSearch(e.target.value)} placeholder=“ID / 姓名 / Nickname / 備忘…” autoComplete=“off” autoCorrect=“off” style={{ …inp, paddingLeft: 36 }} c={c} />
</div>
</div>
...
<div style={{ width: 54, height: 54, borderRadius: 16, flexShrink: 0, background: si ? si.bg : c.pill, border: `2px solid ${si ? si.color : c.border}`, display: “flex”, alignItems: “center”, justifyContent: “center”, fontSize: 22, fontWeight: 800, color: si ? si.color : c.accent }}>
{m.nickname[0]}
</div>
<div style={{ flex: 1 }}>
    ...
    <div style={{ flex: 1, overflowY: "auto", overflowX: "hidden", padding: "14px 16px 32px", WebkitOverflowScrolling: "touch" }}>
      <div style={{ marginBottom: 16 }}>
        <div style={{ fontSize: 9, letterSpacing: 3, color: c.sub, fontWeight: 700, marginBottom: 8, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <button onClick={() => { if (editCrewInfo) { if (tempCrewInfo.nickname.trim()) patchCrew(m.id, tempCrewInfo); setEditCrewInfo(false); } else { setTempCrewInfo({ name: m.name, nickname: m.nickname, seniority: m.seniority }); setEditCrewInfo(true); } }} style={{ background: "none", border: "none", color: c.accent, fontSize: 12, fontWeight: 700, cursor: "pointer" }}>{editCrewInfo ? "💾 儲存" : "✏ 編輯"}</button>
        </div>
        {editCrewInfo ? (
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          ...
          <div style={{ background: c.cardAlt, border: `1px solid ${c.border}`, borderRadius: 12, padding: "10px 14px", fontSize: 13, color: c.sub, lineHeight: 1.8 }}>
            <span style={{ color: c.text, fontWeight: 700 }}>{m.nickname}</span> · {m.name}<br />期別 {m.seniority} · #{m.id}
          </div>
        )}
      </div>
      ...
      <div style={{ marginTop: 32, borderTop: `1px solid ${c.border}`, paddingTop: 20 }}>
        ...
        {confirmDelCrew ? (
          ...
          <button onClick={() => setConfirmDelCrew(true)} style={{ width: "100%", background: "transparent", color: "#FF453A", border: "1px solid rgba(255,69,58,0.35)", borderRadius: 12, padding: "11px", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>🗑 刪除此組員 Delete Crew Member</button>
        )}
      </div>
    </div>
  </div>
...
{view === “quicklog” && (
<QuickLogView crew={crew} routes={routes} setRoutes={setRoutes} initialForm={qlInitialForm} editFlightId={qlEditFlightId} onSave={handleSaveLog} onBack={() => { setView(profileId ? “profile” : “dashboard”); setQlEditFlightId(null); }} dark={dark} c={c} profileId={profileId} allTags={allTags} />
)}
{view === “profile” && ProfView()}
{view === “mylog” && (
<MyLogView flights={flights} crew={crew} username={username} onBack={() => setView(“dashboard”)} onGoProfile={(id) => { setProfileId(id); setView(“profile”); }} onEdit={(f) => { openQL(null, f); }} c={c} />
)}
...
{view === “settings” && (
<SettingsView onBack={() => setView(“dashboard”)} c={c} dark={dark} setDark={setDark} username={username} onLogout={logout} onExport={exportJSON} onGoGuide={() => setView(“guide”)} onGoStats={() => setView(“stats”)} defaultAircraft={defaultAircraft} setDefaultAircraft={setDefaultAircraft} defaultPosition={defaultPosition} setDefaultPosition={setDefaultPosition} customTags={customTags} setCustomTags={setCustomTags} onImport={handleImport} routes={routes} setRoutes={setRoutes} flights={flights} />
)}
</div>
</>
);
